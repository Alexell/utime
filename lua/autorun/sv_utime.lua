...
if not sql.TableExists( "utime" ) then
	sql.Query( "CREATE TABLE IF NOT EXISTS utime ( id INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, player TEXT NOT NULL, totaltime INTEGER NOT NULL, lastvisit INTEGER NOT NULL );" )
end

function onJoin( ply )
	local sid = ply:SteamID()
	local row = sql.QueryRow( "SELECT totaltime, lastvisit FROM utime WHERE player = '" .. sid .. "';" )
	local time = 0 
